...
b2DebugDraw.prototype.DrawPolygon = function (vertices, vertexCount, color, selected) {
    if (selected) {
      ...
      this.m_sprite.lineWidth = this.m_lineThicknessSelected;
  		...
  		this.m_sprite.strokeStyle = this.m_bodyLineColor;//this.ColorStyle(color, this.m_alpha);
...
b2DebugDraw.prototype.DrawSolidPolygon = function (vertices, vertexCount, color, selected) {
    if (selected) {
      this.m_sprite.strokeStyle = this.m_colorSelected;
      this.m_sprite.lineWidth = this.m_lineThicknessSelected;
      //this.m_sprite.fillStyle = this.m_fillColorSelected;
    } else {
  		this.m_sprite.strokeStyle = this.m_bodyLineColor;//this.ColorStyle(color, this.m_alpha);
	  	this.m_sprite.lineWidth = this.m_lineThickness;
		  //this.m_sprite.fillStyle = this.m_fillColor;//this.ColorStyle(color, this.m_fillAlpha);
		}
...
//      this.m_sprite.fillStyle = this.m_fillColorSelected;
  		...
  		this.m_sprite.strokeStyle = this.m_bodyLineColor;//this.ColorStyle(color, this.m_alpha);
	  	this.m_sprite.lineWidth = this.m_lineThickness;
//      this.m_sprite.fillStyle = this.m_fillColor;
	  }
	  this.m_sprite.fillStyle = this.ColorStyle(color, this.m_fillAlpha);
...
b2DebugDraw.prototype.DrawTransform = function (xf) {
		...
		if (this.m_transformOnlyX) this.m_sprite.strokeStyle = this.m_transformOnlyXColor;
    else this.m_sprite.strokeStyle = this.ColorStyle(new b2Color(255, 0, 0), this.m_alpha);
	  this.m_sprite.beginPath();
    ...
    if (this.m_transformOnlyX) return;
...
b2DebugDraw.prototype.m_lineThickness =  0.5;
b2DebugDraw.prototype.m_lineThicknessSelected = 2.0;
